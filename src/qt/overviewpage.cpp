...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeArtaxAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeArtaxAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeArtaxAmount = strAnonymizeArtaxAmount.remove(strAnonymizeArtaxAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeArtaxAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeArtaxAmount*COIN) nMaxToAnonymize = nAnonymizeArtaxAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeArtaxAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeArtaxAmount));
        strAnonymizeArtaxAmount = strAnonymizeArtaxAmount.remove(strAnonymizeArtaxAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeArtaxAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
