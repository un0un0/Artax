...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeArtaxAmount"))
        settings.setValue("nAnonymizeArtaxAmount", 1000);
    nAnonymizeArtaxAmount = settings.value("nAnonymizeArtaxAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeArtaxAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeArtaxAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeArtaxAmount:
            return QVariant(nAnonymizeArtaxAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeArtaxAmount:
            nAnonymizeArtaxAmount = value.toInt();
            settings.setValue("nAnonymizeArtaxAmount", nAnonymizeArtaxAmount);
            emit AnonymizeArtaxAmountChanged(nAnonymizeArtaxAmount);
            break;
        default:
            break;
        }
