...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Artax.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Artaxd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeartaxamount=<n> " + _("Keep N Artax anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Artax is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Artax is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Artax version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Artax") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Artax to complete") << "\n";
    ...
    nAnonymizeArtaxAmount = GetArg("-anonymizeartaxamount", 0);
    if(nAnonymizeArtaxAmount > 999999) nAnonymizeArtaxAmount = 999999;
    if(nAnonymizeArtaxAmount < 2) nAnonymizeArtaxAmount = 2;
    ...
    LogPrintf("Anonymize Artax Amount %d\n", nAnonymizeArtaxAmount);
